...
export const CreateProductVariantForm = ({
  ...
  const handleChangeTab = (update: Tab) => {
    ...
    if (tabOrder.indexOf(update) < tabOrder.indexOf(tab)) {
      // going backwards, no validation
      setTabState((prev) => ({
        ...prev,
        [tab]: "not-started" as ProgressStatus,
        [update]: "in-progress" as ProgressStatus,
      }))
      setTab(update)
      return
    }

    // validate tabs from current to update
    ...
    for (const t of tabs) {
      if (t === Tab.DETAIL) {
        if (
          ...
          setTabState((prev) => ({
            ...prev,
            [t]: "in-progress" as ProgressStatus,
          }))
          setTab(t)
          return
        }
