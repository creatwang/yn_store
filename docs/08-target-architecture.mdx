...
| 一体联调 / 对齐生产 | `pnpm dev:admin-on-server` | **要**（脚本内会 build） | `http://localhost:9000/app/` |
| 生产 / Docker / 仅起 server | `pnpm build:backend` 后启动 server | **要** | `http://localhost:9000/app/` |
...
**API 地址（始终）**：`http://localhost:9000/api/*`（与 Admin 用哪个端口无关）
...
开发 pnpm dev:
  ...
  :4321  store-web  →  商城（独立）

生产 / 联调:
  :9000  server     →  /api/*  +  /app/*（public/app 静态）
```
...
| 与线上一致 | 主机不同（5173 vs 9000），路径同为 `/app` | 开发即 9000/app |
