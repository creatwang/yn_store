...
**application_method**: id (PK, `proappmet`), value (numeric), currency_code, max_quantity, apply_to_quantity, buy_rules_min_quantity, promotion_id (FK)

**campaign**: id (PK, `procamp`), name, description, campaign_identifier, starts_at, ends_at

**campaign_budget**: id (PK, `probudg`), currency_code, limit, used (default: 0), attribute, campaign_id (FK)

**campaign_budget_usage**: id (PK, `probudgus`), attribute_value, used (default: 0), budget_id (FK)
...
**notification**: id (PK, `noti`), to, from, channel, template, data (jsonb), provider_data (jsonb), trigger_type, resource_id, resource_type, receiver_id, original_notification_id, idempotency_key (UNIQUE), external_id, status (`pending`/`sent`/`failed`)
...
| cart_promotion | cart.id | promotion.id | 购物车 ↔ 促销 |
