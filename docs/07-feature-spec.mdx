...
```mermaid
graph TB
    Admin["apps/admin<br/>Vite + React Router"]
    Store["apps/store-web<br/>Astro"]
    RPC["Hono RPC<br/>hc AppType"]
...
| 层级 | 技术 | 职责 |
|------|------|------|
| `apps/server` | Hono + Bun/Node/CF/Vercel 入口 | REST API、认证、业务逻辑、静态资源（生产） |
| `apps/admin` | Vite + React Router + @medusajs/ui | 管理后台 UI |
| `apps/store-web` | Astro | C 端商城 |
...
生产 All-in-One：`entry.bun.ts` 同时 serve `/api/*`、`/admin/*` 静态、`/*` Storefront。
