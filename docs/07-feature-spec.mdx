...
> **目标架构（已实施）**：见 [`08-target-architecture.mdx`](./08-target-architecture.mdx) — **`server`** 提供 `/api` + `/app`（`public/app`）；**`store-web` 独立部署**。

```mermaid
graph TB
    Admin["apps/admin<br/>Vite 源码"]
    Store["apps/store-web<br/>Astro 独立部署"]
...
| `apps/store-web` | Astro（SSG/SSR/hybrid） | C 端，**独立**部署，经 `PUBLIC_API_URL` 调 API |
...
```bash
pnpm install
pnpm dev                    # 并行启动 server + admin + store-web
