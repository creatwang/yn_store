/** Ported from medusajs/medusa packages/medusa/src/api/utils/validators.ts */
import { z, type ZodObject, type ZodOptional, type ZodType } from "zod"

export const WithAdditionalData = <T extends ZodObject<any>>(
  originalSchema: T,
  modifyCallback?: (schema: T) => ZodObject<any>
) => {
  return (additionalDataValidator?: ZodOptional<ZodType<any>>) => {
...
export const createFindParams = ({
  ...
  return selectParams.merge(
    z.object({
      offset: z.preprocess(
        (val) => {
          if (val && typeof val === "string") return parseInt(val)
          return val
        },
        z.number().optional().default(offset ?? 0)
      ),
      limit: z.preprocess(
        (val) => {
          if (val && typeof val === "string") return parseInt(val)
        ...
        z.number().optional().default(limit ?? 20)
      ),
...
export const createOperatorMap = (type?: z.ZodType, valueParser?: (val: any) => any) => {
  if (!type) type = z.string()
